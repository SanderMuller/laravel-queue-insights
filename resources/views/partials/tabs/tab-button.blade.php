...
<button type="button"
        ...
        x-bind:class="tab==='{{ $name }}' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-900'"
        class="inline-flex items-center gap-1.5 border-b-2 px-3 py-2 text-sm font-medium tabular-nums">
    {{ $label }}
    @if($badge !== null)
        <span class="text-xs font-normal text-gray-400">{{ $badge }}</span>
    @endif
</button>

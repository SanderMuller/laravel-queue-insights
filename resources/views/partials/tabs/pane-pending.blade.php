...
@else
    ...
    @if(count($pendingRows) > 0)
        <h3 class="mb-2 text-sm font-semibold tracking-wide text-gray-500 dark:text-gray-300">Pending now <span class="font-normal text-gray-400 dark:text-gray-400 tabular-nums">({{ count($pendingRows) }})</span></h3>
        <div class="mb-5 rounded-lg bg-white ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
            <div class="grid grid-cols-12 items-center gap-4 border-b border-gray-950/5 px-4 py-2 text-sm font-medium text-gray-500 dark:border-white/10 dark:text-gray-300">
                <div class="col-span-5">Job</div>
                <div class="col-span-3">Queue</div>
    ...
    @if(count($delayedRows) > 0)
        <h3 class="mb-2 text-sm font-semibold tracking-wide text-indigo-700 dark:text-indigo-300">Delayed <span class="font-normal text-indigo-500 dark:text-indigo-400 tabular-nums">({{ count($delayedRows) }})</span></h3>
        <div class="rounded-lg bg-white ring-1 ring-indigo-600/15 dark:bg-gray-900 dark:ring-indigo-400/30">
            <div class="grid grid-cols-12 items-center gap-4 border-b border-indigo-200/60 px-4 py-2 text-sm font-medium text-indigo-700/80 dark:border-indigo-400/20 dark:text-indigo-300">
                <div class="col-span-5">Job</div>
                <div class="col-span-3">Queue</div>
                <div class="col-span-2 text-right">Queued</div>

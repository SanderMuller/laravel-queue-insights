...
- **The dashboard could take the whole application down at boot.** The service provider registered its Livewire components from inside its own `boot()`, and `Livewire::component()` resolves `livewire.finder` — a binding only Livewire's service provider registers. Boot order between the two providers is not guaranteed, so an application that booted this provider first, or one running with package discovery disabled, died with `Target class [livewire.finder] does not exist` before it could serve anything.
  
  Registration now happens from an `app->booted` callback, after every provider has registered. If the bindings still are not there, the failure is logged and the dashboard stays unregistered rather than aborting the boot. Nothing to change on your side; `dashboard.enabled` and the component names are unchanged.
  

**Full Changelog**: https://github.com/SanderMuller/laravel-queue-insights/compare/0.33.0...0.33.1
...
- **Sentry deep-link for scheduled-task failures.** When Sentry captures a scheduled-task exception, the per-run drilldown modal now shows a **View in Sentry** button and a `Sentry:` line in the markdown export. The link is keyed by Sentry's event ID (not a distributed trace ID, since scheduled tasks run outside a trace context). No extra config is needed beyond `sentry.organization`. The package installs a `beforeSend` hook to record the event ID at capture time, so the link is race-safe even when multiple exceptions are captured on the same hub in quick succession.
  
- **Inner throw-site in scheduler run modals.** The exception block for a failed scheduled run now shows the root-cause file, line, and stack-trace tail (`inner_file` / `inner_line` / `inner_trace_tail` from the deepest `getPrevious()` chain). Previously the exception block pointed at the `ScheduleRunCommand` wrapper frame; now it shows the real throw site.
  

### Changed

- Scheduler alert notification URLs (Slack `Run URL` field, mail action button) now include a `#qi-schedule` fragment, so clicking a scheduler alert link automatically activates the Schedule tab on the dashboard.

<!-- benchmark-start -->
<!-- benchmark-end -->
**Full Changelog**: https://github.com/SanderMuller/laravel-queue-insights/compare/0.28.0...0.29.0

## 0.28.0 - 2026-06-26

<!-- verified-sha: 97e23eb5c7eef3ebc15e0e32a3c38db6e9d3720c -->
Operators running high-frequency scheduled tasks on Vapor (or any per-minute scheduler with cold-start / invocation jitter) saw spurious `scheduled_task_missed` alerts: the sweeper judged a fire the instant its cron timestamp passed, before a late-but-still-valid `Starting` event could land. This release makes the missed detector wait out the drift grace and debounce isolated misses.

### Added

- `scheduler.sweeper.min_consecutive_misses` (default `2`) — debounces missed-task alerts. The synthetic `missed` row is still recorded for the dashboard, but `ScheduledTaskMissed` only dispatches once that many consecutive expected fires go unobserved. Set to `1` to alert on every isolated miss.

### Changed

- The missed-run sweeper now waits out the full drift grace before judging a fire: an expected fire is evaluated only once `now ≥ expected_at + drift_seconds`, so a `Starting` that arrives late-but-within-drift (Vapor cold start, EventBridge jitter) still suppresses the miss. The liveness gauge (`scheduled_sweeper_age_seconds`) is unaffected — `last_swept_ms` still tracks the actual sweep time.

### Fixed

- Stopped false-positive `scheduled_task_missed` alerts on per-minute and high-frequency schedules where a valid `Starting` event landed within the drift window but after the sweep had already flagged the fire as missed.

**Full Changelog**: https://github.com/SanderMuller/laravel-queue-insights/compare/0.27.0...0.28.0

## 0.27.0 - 2026-06-17

<!-- verified-sha: 782878cc5c3561ef066b40a01032c4bb1cefc351 -->
### Added

- **"View in Sentry" deep-link in the failed-job modal.** When a failed job's payload carries sentry-laravel trace data (`sentry_trace_parent_data` / `sentry_baggage_data`), the modal renders a **View in Sentry** button and the Markdown export gains a `Sentry:` line, both linking to the Sentry issue filtered by that job's trace id. Opt-in via a new top-level `sentry` config block — set `organization` (your org slug, `QUEUE_INSIGHTS_SENTRY_ORG`) and the button appears; leave it null to hide it. The link targets the issue stream rather than the performance/trace view, so it resolves even when the trace was sampled out (error capture is independent of `traces_sample_rate`).

**Full Changelog**: https://github.com/SanderMuller/laravel-queue-insights/compare/0.26.0...0.27.0

## 0.26.0 - 2026-06-09

<!-- verified-sha: 9ebe17440312b2abe1b7f65603c420b6c1dc5a18 -->
The dashboard was restyled to a calmer, Laravel-Cloud-inspired look. Same data and routes — the change is purely visual, so upgrading is a plain `composer update` with no migration.

### Changed

- Reskinned the dashboard to a Laravel-Cloud-inspired light theme: LC card headers with icon squares, softer borders, tightened type and spacing, and a calmer canvas.
- Reworked the sidebar into an icon-led nav, with the active section now marked by a left-border accent instead of a background tint.
- Rebuilt the filter bar on Laravel Cloud search + filter components, and softened the filter and pagination controls to the LC corner radius.
- Restyled the headline LIVE panel with LC metric-card vertical dividers, dropped the aurora gradient, and added a styled checkbox.
- Aligned all modals (including the schedule-run modal) to the cloud card radius and border, dropping the accent stripe.
- Recased the notice banners (Horizon, watchdog) to the calm LC notice tone and gave the job-classes, throughput, schedule, and alert-rules sections LC icon-headers.

### Fixed

- `RedisMemoryUsage::totalBytes()` now guards its cached value with a real `is_int()` check. A non-int cache entry previously slipped past the type declaration and could throw a `TypeError` at runtime.

**Full Changelog:** https://github.com/SanderMuller/laravel-queue-insights/compare/0.25.0...0.26.0

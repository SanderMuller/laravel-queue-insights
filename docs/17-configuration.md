...
| `QUEUE_INSIGHTS_REDIS` | `default` | Redis **connection name** from `config/database.php` → `redis`. Not a database number. A name with no matching connection is rejected at boot. |
...
| `failure_context.capture_app_context` | `true` | Snapshot the visible Laravel `Context` facade. Hidden context is never captured, since it holds package internals such as `qi_origin`. |
